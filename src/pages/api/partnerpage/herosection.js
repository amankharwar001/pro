    ...
    if (req.method === 'POST') {
      ...
      if (!title || !heading || !text ) {
        return res.status(400).json({
          success: false,
          message: ' (title, heading, text, btn, btnLink) are required.',
        });
      }

      // Check if a record already exists
      let heroSection = await HeroSectionPatnerPage.findOne();
      ...
      if (heroSection) {
        ...
        // Create a new record
        heroSection = await HeroSectionPatnerPage.create({
          title,
          heading,
          text,
          btn,
      ...
      const heroSection = await HeroSectionPatnerPage.findOne();
