...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const section = await Section8.findOne();
      if (!section) {
    ...
    try {
      ...
      let section = await Section8.findOne();
      if (!section) {
    ...
    // Create a new Section8 record only if it doesn't exist
    try {
      const { heading, content, btn, btnLink } = req.body;
      ...
      // Check if the Section8 record already exists
      let section = await Section8.findOne();
      if (section) {
      ...
      section = await Section8.create({ heading, content, btn, btnLink });
