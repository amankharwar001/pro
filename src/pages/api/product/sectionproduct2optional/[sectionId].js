...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    try {
      ...
      let existingEntry = await Section2Optional.findOne({
  ...
  // Handle POST request to create a new entry if no entry exists
    ...
    try {
      ...
      const existingEntry = await Section2Optional.findOne({
        where: { heroSectionId: sectionId },
      });
      ...
      if (existingEntry) {
        ...
        const newEntry = await Section2Optional.create({
          title,
  ...
  // Handle GET request to fetch the only entry
    ...
    try {
      const entry = await Section2Optional.findOne({
        where: { heroSectionId: sectionId },
      });
