...
const HeroSectionForm = ({ onSubmitId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/contactpage/herosection`);
        if (res.ok) {
    ...
    <div className="mx-auto p-4 bg-white shadow-md rounded flex flex-row-reverse gap-10 items-center">
      <div>
        ...
        <div className="bg-slate-100 p-4 rounded-md mt-2">
          <ImageUploader referenceType="herosection_contact_page" />
        </div>
      </div>
      <form onSubmit={handleSubmit} className="space-y-4 w-full">
        ...
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? "Submitting..." : formData.id ? "Update" : "Create"}
        </button>
      </form>
    </div>
  );
