...
const Testimonial = ({ apidata }) => {
  ...
  const primaryImage = images?.find(img => img.referenceType === 'homepage_testimonial_primary');
  const otherImages = images?.filter(img => img.referenceType !== 'homepage_testimonial_primary');

  console.log("primary images show is here",images)
  const positions = [
    { className: 'absolute right-[18%] md:right-[28%] translate-x-2/4 top-2/4 -translate-y-2/4', size: 'w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10' },
    { className: 'absolute right-[5%] md:right-[8%] translate-x-2/4 top-[18%] md:top-1/4 -translate-y-2/4', size: 'w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12' },
    { className: 'absolute right-[15%] -translate-x-[10%] top-[25%] -translate-y-2/4', size: 'w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14' },
      ...
      <div className="text-center p-8 bg-gray-50 pt-20 py-10">
        ...
        <Zoom triggerOnce>
          <div className="container relative h-72 -mb-10 max-w-[1200px]">
            {/* Primary Image */}
            {primaryImage && (
              <div className='absolute right-2/4 translate-x-2/4 top-[45%] md:top-2/4 -translate-y-2/4'>
                <Image
                  src={`/${primaryImage.filePath}`}
                  alt={primaryImage.altText || 'Primary Testimonial Image'}
                  className='bg-slate-200 rounded-full object-cover w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24'
                  width={96}
                  height={96}
                />
              </div>
            ...
            {otherImages?.map((image, index) => (
              <div key={image.id} className={positions[index % positions.length].className}>
                <Image
                  src={`/${image.filePath}`}
                  alt={image.altText || `Testimonial image ${index + 1}`}
                  className={`bg-slate-200 rounded-full object-cover ${positions[index % positions.length].size}`}
                  width={96}
                  height={96}
                />
              </div>
